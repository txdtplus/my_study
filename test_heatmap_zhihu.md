# 采用pytorch生成热力图(heatmap)

<!-- TOC -->

- [采用pytorch生成热力图(heatmap)](#采用pytorch生成热力图heatmap)
...
本文采用 pytorch_grad_cam 库和 torchvision 库来计算热力图。

## Requirements
本文采用 PyTorch 1.13.1 和 Torchvision 0.14.1 进行试验。并且还需要安装 pytorch_grad_cam 库，采用以下命令安装：
